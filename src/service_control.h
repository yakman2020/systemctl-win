...
private:
    ...
    int restart_sec;             // time to sleep before restarting a service. Default 100ms
    int timeout_start_sec;       // time to wait for startup. -1 is "infinity", Defaults to DefautTimeoutStartSec
...
public:
    ...
    static class SystemDUnit *ReadServiceUnit(string name) {
      ...
      void LoadPool() { return true; };
      void SavePool() { return true; };
